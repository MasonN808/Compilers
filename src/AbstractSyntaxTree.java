    ...
    public void rearrangeTree(Node node){// post order traversal
            for(Node each : node.children){
                if (each.name.equals("ifStatement")|each.name.equals("whileStatement"))
                    each.children.get(1).children.add(each.children.get(0)); //Add child 0 to child of child 1
                    each.children.get(1).children.add(each.children.get(2)); //Add child 2 to child of child 1
                    each.children.get(0).parent = each.children.get(1); //assign the parent of child 0 to be child 1
                    each.children.get(2).parent = each.children.get(1); //assign the parent of child 2 to be child 1
